...
def main():
    ...
    page = st.sidebar.radio("Navegação", ["🔍 Recomendações", "🤖 Chat com IA"])
    ...
    if page == "🔍 Recomendações":
        ...
        if search_query:
            matches = fuzzy_search(search_query, movie_titles, limit=10)
            if matches:
                selected_movie = st.selectbox("Filmes encontrados:", matches, key="search_select")
            else:
                st.warning("Nenhum filme encontrado.")
                return
        else:
            selected_movie = st.selectbox("Ou escolha da lista:", [""] + movie_titles, key="list_select")
        ...
        if st.button("Gerar Recomendações", type="primary", use_container_width=True):
            ...
            with st.spinner("🤖 Analisando e gerando explicações com IA..."):
                ...
                with tab2:
                    ...
                    st.markdown(f'<img src="data:image/png;base64,{chart_b64}" style="width:100%"/>', unsafe_allow_html=True)
                ...
                with tab3:
                    ...
                    if corpus_texts:
                        ...
                        st.markdown(f'<img src="data:image/png;base64,{wc_b64}" style="width:100%"/>', unsafe_allow_html=True)
                        st.caption("📌 Palavras maiores = termos mais frequentes nos filmes recomendados")
                    else:
                        st.warning("Dados insuficientes para gerar nuvem de palavras")

    elif page == "🤖 Chat com IA":
        st.markdown("### Converse com o assistente para descobrir filmes")
        st.markdown("Descreva o que você quer assistir e a IA encontra para você.")
        ...
        user_input = st.chat_input("Ex: Quero filmes de ficção científica com viagem no tempo...")
        if user_input:
            ...
            with st.chat_message("assistant"):
                with st.spinner("Pensando..."):
                    response = st.session_state.agent.invoke({"messages": [("human", user_input)]})
